...
import { CheckCircle, MapPin, Mail, Phone, ArrowRight, ArrowLeft, X, Sparkles } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
        ...
        >
          <button
            ...
            className="btn-primary px-8 py-4.5 text-xs font-extrabold uppercase tracking-widest flex items-center gap-3 cursor-pointer shadow-lg hover:shadow-brand-teal/10 transition-all duration-300"
          >
            <Sparkles className="w-4 h-4 text-brand-gold animate-pulse" />
            <span>{lang === 'en' ? 'Start Consultation Request' : 'ابدأ طلب استشارة مجانية'}</span>
            {lang === 'en' ? <ArrowRight className="w-4 h-4" /> : <ArrowLeft className="w-4 h-4" />}
          </button>
        </motion.div>

        {/* Offices Side-by-Side Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="glass-card p-6 border-brand-teal/10 hover:border-brand-gold/25 transition-all duration-300 bg-[#1E2A33]/20 text-left"
          >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="absolute inset-0 bg-[#0C0F12]/85 backdrop-blur-md"
            />
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-[#0F1318] border border-brand-teal/15 rounded-3xl p-6 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto z-10 dot-grid noise-overlay text-left"
            >
              
              {/* Close Button */}
              <button
                onClick={() => setIsModalOpen(false)}
                className="absolute top-4 right-4 md:top-6 md:right-6 p-2 rounded-full bg-white/5 border border-white/10 text-brand-ghost hover:text-white hover:bg-white/10 transition-all cursor-pointer z-20"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="mb-6 pr-8">
                <h3 className="font-display font-bold text-white text-xl sm:text-2xl mb-1">
                  {lang === 'en' ? 'Consultation & Inquiry Form' : 'نموذج طلب الاستشارة والاستفسار'}
                </h3>
                <p className="text-xs sm:text-sm text-brand-ghost font-medium">
                  {lang === 'en' ? 'Provide your details to customize your premium curtain system.' : 'يرجى تقديم تفاصيل مشروعك للحصول على حلول ستائر مخصصة.'}
                </p>
              </div>
              ...
              {isSubmitted ? (
                <div className="py-12 px-6 flex flex-col items-center text-center gap-6 animate-fade-up">
                  ...
                  <div className="flex gap-4 mt-4">
                    ...
                    >
                      {lang === 'en' ? 'Submit Another' : 'إرسال طلب آخر'}
                    </button>
                    <button 
                      onClick={() => setIsModalOpen(false)}
