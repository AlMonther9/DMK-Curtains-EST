...
export default function Navbar({ t, lang, handleLangToggle, scrolled }: NavbarProps) {
  ...
  return (
    <header
      className={`fixed left-0 right-0 z-50 transition-all duration-500 flex justify-center px-4 ${scrolled ? "top-4" : "top-6"
        }`}
    >
      <div className={`w-full max-w-5xl rounded-full transition-all duration-500 ${scrolled
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <a href="mailto:almonther.art@gmail.com" className="btn-primary py-2 px-4 text-xs font-bold rounded-full">
            {t.contact.sendBtn}
          </a>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-4 right-4 mt-2 bg-[#131820]/95 backdrop-blur-md border border-brand-teal/20 rounded-2xl p-6 flex flex-col gap-4 shadow-card-dark animate-fade-up z-50">
          ...
          <a href="mailto:[EMAIL_ADDRESS]" className="btn-primary py-2 px-4 text-xs font-bold rounded-full">
            {t.contact.sendBtn}
          </a>
        </div>
      )}
